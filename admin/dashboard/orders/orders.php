...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_as_paid']) && isset($_POST['OrderID'])) {
    $orderID = intval($_POST['OrderID']);
    ...
    try {
        ...
        echo json_encode(['success' => false, 'message' => 'Lỗi khi cập nhật trạng thái đơn hàng: ' . $e->getMessage()]);
    }
    exit;
}
?>
...
<head>
    ...
    function toggleDetails(orderId, button) {
        ...
        if (details.style.display === "none" || details.style.display === "") {
            ...
            button.innerText = "Đóng";  // Đổi chữ nút thành "Đóng"
        } else {
            details.style.display = "none";
            button.innerText = "Mở";  // Đổi chữ nút thành "Mở"
        }
    }
</script>
...
<th>Thanh Toán</th>
    ...
    echo "<td><button class='toggle-button' onclick='toggleDetails($order_id, this)'>Mở</button></td>";

    // Kiểm tra nếu trạng thái đơn hàng là "completed", thì không hiển thị nút thanh toán
    if ($order['status'] !== 'completed') {
        echo "<td><button class='toggle-button' onclick='markAsPaid($order_id)'>Thanh toán</button></td>";
    } else {
        echo "<td>Đã thanh toán</td>";  // Hiển thị thông báo nếu đã thanh toán
    }
...
function loadEditOrderForm() {
    ...
    if (selectedRadio) {
        ...
        $.ajax({
            ...
            error: function() {
                alert("Đơn hàng đã thanh toán không thể chỉnh sửa.");
            }
        });
    } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (result.success) {
                    ...
                    window.open('dashboard/orders/print_order.php?OrderID=' + orderId, '_blank');  // Mở trang in
                    loadOrders();  // Cập nhật lại danh sách đơn hàng
                } else {
